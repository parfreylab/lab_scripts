...
#it's also necessary to filter out low-abundance ASVs after constructing the sequence table in dada2. dada2 developers do not provide any advice on this front, but two options are outlined below in the section immediately after sequence table construction
...
#mostly this needs doing so that when we reach the sample merging stage, we have no samples that have no merged reads (not allowed by dada2, prevents merging)
#remove samples that have too few reads for merging <=50 reads (arbitrary, but low enough that we're throwing out only samples we might want to exclude on this basis)
#actual sample filtering based on read counts and judgements about "successful" or "unsuccessful" samples can be made after the handoff to phyloseq
#do this after trimming and filtering (creating "filtFs", "filtRs", and "out" objects)
#remove samples that have too few reads already
samples_to_keep <- as.numeric(out[,"reads.out"]) > 100 #or other cutoff #IMPORTANT: needs to be adjusted per-experiment, use your best judgement and remember this is a low pass filter, you don't need to throw out everything yet
#need to redefine objects from above to exclude samples you excluded in this section
fnFs <- sort(list.files(path, pattern="_R1_001.fastq", full.names = TRUE))[samples_to_keep]
fnRs <- sort(list.files(path, pattern="_R2_001.fastq", full.names = TRUE))[samples_to_keep]
...
hist(otu_singleton_rowsums[,1], breaks=10000, xlim = c(0,200), xlab="# Reads in ASV") #plot of above
length(which(otu_singleton_rowsums <= 1)) #how many are there with more than N reads? (N=1 in example)
length(otu_rowsums)
...
colnames(taxa) <- c("Rank1", "Rank2", "Rank3", "Rank4", "Rank5", "Rank6", "Rank7", "Rank8", "Accession")
...
# Set aside unassigned taxa #with dada2, there may not be any unassigned taxa (i think they are being removed automatically by dada2)
...
##TODO: remove samples that have fewer than N reads
