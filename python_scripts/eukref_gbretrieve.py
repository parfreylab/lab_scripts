...
parser.add_argument('-i','--db',help='Dataset of sarting sequences in fasta format. MUST be in standard GenBank format (>gi|ginumber|gb|accession| ), or have the accession number followed by a space (>accession ), input filename',required=True)
parser.add_argument('-dbnt','--path_to_nt',help='Path to NCBI nt database, string',required=True)
parser.add_argument('-dbsi','--path_to_ReferenceDB',help='Path to Reference database, formated as DNA, string',required=True)
...
parser.add_argument('-idsi','--percent_identity_ReferenceDB',help='percent identity for blast hits against Reference DB, float',required=True)
parser.add_argument('-td','--taxonomy_dict',help='path to taxonomy dictionary - file tax_d.bin',required=True)
parser.add_argument('-gb','--gb_fasta',help='path to the gb_fasta file file gb203_pr2_all_10_28_97p_noorg.fa',required=True)
...
print "Path to database: %s" % db
print "Path to NCBI NT database: %s" % path_to_nt
print "Path to Silva database: %s" % path_to_silva
print "Number of sequences per round of Blast: %s" % num_seq_per_round
print "Number of CPUs: %s" % cpu
print "Target group name: %s" % group_name
#print args

# loads in the tax dictionary
tax_d = pickle.load(open(taxonomy_dict_file, 'rb'))

Renaming_d = {}

os.system("usearch -makeudb_usearch %s -output Reference_DB.udb" % args.path_to_ReferenceDB)
os.system("usearch -makeudb_usearch %s -output gb203_pr2_all_10_28_97p_noorg.udb" % args.gb_fasta)
